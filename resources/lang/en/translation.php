<?php

return [

    "working-hours" => "Öffnungszeiten: 7:30 bis 13:00 - 14:00 bis 18:00",
    "call-us" => "Tel.",
    "phone-number" => "044 829 24 12",
    "phone-number2" =>"079 838 53 76",
    "hotline" => "Hotline",
    "team" => "Team",
    "teams" => "Team",
    "home" => "Startseite",
    "about" => "Über",
    "about-text" => "Wir sind darauf spezialisiert, Ihnen ein breites Spektrum an individuellen Dienstleistungen anzubieten. Egal ob Sie ein Fahrzeug verkaufen oder kaufen möchten. Unsere Dienstleistungen sollen Ihnen das Leben erleichtern. Ihre Bedürfnisse und unsere Expertise schaffen Kundennutzen. Darin sehen wir unsere Aufgabe",
    "about-title" => "WILLKOMMEN BEI CHARAF TRANSPORT",
    "our-advantages" => "unsere Vorteile",
    "purchases" => "ANKAUF",
    "purchases-text" => "Den Nutzfahrzeugverkäufer interessiert vor allem der höchste Preis. Durch permanente Optimierung der Kostenuntergrenze sind wir in der Lage, Preisangriffe zu parieren. Top Preise, sofortige Bezahlung und umgehende Abholung  – das sind die Argumente, die zählen.",
    "sale" => "REMARKETING",
    "sale-text" => "Wiederverkauf und Remarketing ist unsere Passion Haben Sie ein gebrauchtes Nutzfahrzeug zu verkaufen? Wir sind die Spezialisten im Remarketing von gebrauchten Nutzfahrzeugen.Dank unseres internationalen Teams sind wir in der Lage, für jedes Fahrzeug den richtigen Absatzkanal anzubieten.",
    "delivery" => "LIEFERUNG",
    "delivery-text" => "Wir transportieren jede Woche an verschieden Hafen in Europa , Wir bieten unseren Kunden den Transport der Fahrzeuge zu ihrem Standort an. Dank unserem Netzwerk mit internationalen Transporteuren können wir ihren Lkw an viele Standorte zu guten Preisen liefern.",
    "bring-to-border" => "BRING-TO-BORDER",
    "bring-to-border-text" => "Mit unserem Bring-to-Border-Service können Sie sich das kurzfristige Einlösen des Fahrzeugs in der Schweiz ersparen, Wir liefern die Fahrzeuge auf Wunsch auch an die Schweizer Grenze. So können sie sich das einlösen des Fahrzeugs in der Schweiz ersparen. können sie sich das einlösen des Fahrzeugs.",
    "follow-us" => "Folge uns",
    "testimonials" => "Referenzen",
    "shop" => "Shop",
    "services" => "Dienstleistungen",
    "more-about-us"=>"Über Uns",
    "our-specialized-doctor-details"=>"Unser Team",
    "our-latest-news" => "Our Latest News",
    "our-latest-news-description"=>"Our latestnews description",
    "subscribe-your-email"=>"Abonnieren Sie Ihre Email",
    "subscribe-your-email-details" => "",
    "blogs" => "Blogs",
    "contact-us" => "Kontakt",
    "book-an-appointment" => "Book An Appointment",
    "diet-plan" => "Diet Plan",
    "our-weekly-diet-plan" =>"Our Weekly Diet Plan",
    "make-an-appointment" => "Make an Appointment",
    "name" => "Name",
    "email" => "Email",
    "email-address" => "info@charaf-transport.com",
    "phone" => "Telefon",
    "date" => "Date",
    "your-message" => "Ihre Nachricht",
    "submit-now" => "Submit Now",
    "type-of-service" => "Type of Service",
    "weight-lifting" => "Weight Lifting",
    "organic-yoga" => "Organic Yoga",
    "power-yoga" => "Power Yoga",
    "useful-links" => "Useful Links",
    "informations" => "Informations",
    "address" => "Adresse",
    "address-info" => "Arztpraxis Kadir <br>Schaffhauserstrasse 110<br>8152 Glattbrugg",
    "address-details" => "Arztpraxis Kadir, Schaffhauserstrasse 110, 8152 Glattbrugg",
    "phone-info" => "+96179779100",
    "copy-right" => "Copyright © 2022. All Rights Reserved by",
    "how-it-works" => "How It Works",
    "easy-steps-for-happy-life" => "4 Easy Steps For Happy Life",
    "appointment" => "Appointment",
    "analysis" => "Analysis",
    "happy-life" => "Happy Life",
    "what-we-offer" => "What We Offer",
    "diet-services" => "Diet & Nutritionist Services",
    "learn-more" => "erfahren Sie mehr",
    "latest-new" => "",
    "latest-recipe-from-blog" => "Unsere Praxis",
    "contact-info" => "Contact Info",
    "connect-with-us" => "Let's Connect With Us",
    "your-subject" => "Dein Fach",
    "send-message" => "Nachricht senden",
    "pricing-plan" => "Pricing Plan",
    "choose-your-best-plan" => "Choose Your Best Plan",
    "book-now" =>  "Book Now",
    "nutrition-special-feature" => "Nutrition Special Feature",
    "bad-habits" => "Bad Habits",
    "results" => "Results",
    "before-after" => "Before &amp; After",
    "results-description" => "Check out these amazing before-and-after weight loss pictures to give you the motivation you need to take the first step for a healthier you.",
    "footer-text"=>"Facharzt für Kinder und Jugendliche und Praktischer Arzt für Allgemeinmedizin",
    "sitemap"=>"Seitenverzeichnis",
    "opening-hours"=>"Öffnungszeiten",
    "monday" =>"Montag",
    "tuesday" =>"Dienstag",
    "wednesday" => "Mittwoch",
    "thursday" =>"Donnerstag",
    "friday" =>"Freitag",
    "saturday" =>"Samstag",
    "sunday" =>"Sonntag",

    "monday-opening-hours" =>"7:30 bis 13:00 - 14:00 bis 18:00",
    "tuesday-opening-hours" =>"7:30 bis 13:00 - 14:00 bis 18:00",
    "wednesday-opening-hours" => "7:30 bis 13:00 - 14:00 bis 18:00",
    "thursday-opening-hours" =>"7:30 bis 13:00",
    "friday-opening-hours" =>"7:30 bis 13:00 - 14:00 bis 18:00",
    "saturday-opening-hours" =>"Geschlossen",
    "sunday-opening-hours" =>"Geschlossen",

    "get-in-touch"=>"Kontakt",
    "email1" =>"Mail für Ärzte",
    "email2" =>"Mail",
    "email-address1" =>"nfo@charaf-transport.com",
    "email-address2" =>"nfo@charaf-transport.com",
    "specialized-services" => "Spezialisierte Dienstleistungen",
    "services-details" => "Unsere Dienstleistungen",
    "services-det"=>"Einzelheiten zu den Diensten",
    "contact"=>"Kontakt",
    "gallery"=> "Gallerie",
    "subscribe-now"=>"Abonnieren Sie Jetzt",
    "enter-your-email" => "Email eingeben"
];